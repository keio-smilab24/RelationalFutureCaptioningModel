...
class MultiHead(nn.Module):
    ...
    def forward(self, query, key, value):
        ...
        return self.wo(
            torch.cat(
                [self.attention(q, k, v) for q, k, v in zip(query, key, value)], -1
            )
        )
...
class EncoderLayer(nn.Module):
    def __init__(self, d_model, d_hidden, n_heads, drop_ratio):
        ...
        self.feedforward = ResidualBlock(
            FeedForward(d_model, d_hidden), d_model, drop_ratio
        )
...
class DecoderLayer(nn.Module):
    def __init__(self, d_model, d_hidden, n_heads, drop_ratio):
        ...
        self.attention = ResidualBlock(
            MultiHead(d_model, d_model, n_heads, drop_ratio), d_model, drop_ratio
        )
        self.feedforward = ResidualBlock(
            FeedForward(d_model, d_hidden), d_model, drop_ratio
        )
...
class Decoder(nn.Module):
    def __init__(self, d_model, d_hidden, vocab_size, n_layers, n_heads, drop_ratio):
...
class MTransformer(nn.Module):
    def __init__(self, cfg: MartConfig):
        ...
        self.encoder = Encoder(
            vfeat_size, d_model, d_hidden, n_layers, n_heads, drop_ratio
        )
        self.decoder = Decoder(
            d_model, d_hidden, self.vocab_size, n_layers, n_heads, drop_ratio
        )
        self.loss_func = (
            LabelSmoothingLoss(cfg.label_smoothing, cfg.vocab_size, ignore_index=-1)
            if "label_smoothing" in cfg and cfg.label_smoothing > 0
    ...
    def decode(
        ...
        caption_loss = self.loss_func(
            prediction_scores.view(-1, self.cfg.vocab_size), text_input_labels.view(-1)
        )  # float
        return caption_loss, prediction_scores

    def forward(
        self, video_features, video_masks, text_input_ids, text_masks, text_input_labels
    ):
        """
        Args:
            video_features: (N, Lv, Dv)
        ...
        """
        encoder_layer_outputs = self.encode(
            video_features, video_masks
        )  # [(N, Lv, D), ] * num_hidden_layers
        caption_loss, prediction_scores = self.decode(
            text_input_ids, text_masks, text_input_labels, encoder_layer_outputs, None
        )  # float, (N, Lt, vocab_size)
        return caption_loss, prediction_scores
